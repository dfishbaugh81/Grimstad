...
{
    RDLCLayout = './StandardPurchOrder.rdl';
        ...
        dataitem("Purchase Header"; "Purchase Header")
        {
            ...
            dataitem("Purchase Line"; "Purchase Line")
            {
                        ...
                        If PurchCommentLine.FindFirst() then
                            repeat
                                if StrLen(PurchCommLineText) < 1024 then begin
                                    if StrLen(PurchCommLineText) + StrLen(PurchCommentLine.Comment) + 1 <= 1024 then
                                        PurchCommLineText := PurchCommLineText + ' ' + PurchCommentLine.Comment;
                                end;
                ...
                begin
                    ...
                    ItemNo := "No.";
                    Clear(CustRefPo);
                    if SalesLine.Get("Document Type"::Order, "Purchase Line"."Sales Order No.", "Purchase Line"."Sales Order Line No.") then
                        if SalesHeader.Get("Document Type"::Order, "Purchase Line"."Sales Order No.") then
                            CustRefPo := 'Reference PO #: ' + SalesHeader."External Document No.";
                    ...
                    If PurchComment.FindFirst() then
                        repeat
                            if StrLen(PurchCommHeaderText) < 1024 then begin
                                if StrLen(PurchCommHeaderText) + StrLen(PurchComment.Comment) + 1 <= 1024 then
                                    PurchCommHeaderText := PurchCommHeaderText + ' ' + PurchComment.Comment;
                            end;
