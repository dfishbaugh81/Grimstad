    ...
    dataset
    {
        dataitem("IWX LP Header"; "IWX LP Header")
        {
            ...
            begin
                ...
                if SalespersonPurchaser.Get(Header."Salesperson Code")then SalesPerson:=SalespersonPurchaser.Name;
                clear(SalesHeaderText);
            ...
            begin
                if "Source Document" = "Source Document"::" " then Evaluate("Source Document", GetFilter("Source Document"));
                if "Source No." = '' then Evaluate("Source No.", GetFilter("Source No."));
                case "Source Document" of "Source Document"::Shipment: begin
                    WhHeader.Get("Source No.");
                    myRecRef.GetTable(WhHeader);
                    dshipPackMan.getPackageData(myRecRef, ptrecLineBuffer);
                end;
                "Source Document"::"Sales Order": begin
