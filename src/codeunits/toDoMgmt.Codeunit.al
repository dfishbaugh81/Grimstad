codeunit 50102 "toDoMgmt"
{
    ...
    var SLine: Record "Sales Line";
    PLine: Record "Purchase Line";
    procedure GetTotalRecCount(var ToDo: Record "To-Do"; userCode: code[20]): Integer var
    ...
    procedure VerifySplit(whseAct: Record "Warehouse Activity Line"; var currLine: Integer; var currNo: Code[20]; var currType: Enum "Warehouse Activity Type"): Boolean var
        typeMatch: Boolean;
        DocNoMatch: Boolean;
        LineNoMatch: Boolean;
    begin
    ...
    procedure ValidateBomStatus(prodBom: Record "Production BOM Header"; prodStatus: enum "Production Order Status"): Boolean var
        success: Boolean;
    begin
        if prodBom.FindFirst()then repeat prodBom.Validate(Status, prodStatus);
                success:=prodBom.Modify(true);
            until prodBom.Next() = 0;
        exit(success);
    end;
    procedure RemoveLineFromCertProdBom(prodBomNo: Code[20]; versCode: Code[20]; LineNo: Integer): Boolean var
        success: Boolean;
    ...
    begin
        if PurchCode.Get(SalesLine."Purchasing Code")then if PurchCode."Special Order" = true then if SalesLine.Type = SalesLine.Type::" " then begin
    ...
    begin
        ...
        if ProbReqLine.FindFirst()then repeat NewReqLine.Reset;
            ...
            if sl.FindLast()then begin
                ...
                if pl.FindFirst()then begin
                    ...
                    while PurchOrderLine.Get(pl."Document Type", pl."Document No.", NextLineNo)do begin
                        if PurchOrderLine.Description = RequisitionLine.Description then exit
                        else
                        begin
        ...
        if TempReportSelectionWarehouse.Usage = TempReportSelectionWarehouse.Usage::Pick then begin
            ...
            if WhseActHead.FindFirst()then begin
                ...
                if WhseActLine.Findfirst()then begin
                    if WhseActLine."Source Type" = 37 then Report.Run(50115, ShowRequestPage, false, RecVarToPrint)
                    else
    ...
    begin
        ...
        if SalesCommentLine.FindFirst()then repeat AddItemTextToPurchCommentLine(PurchOrderLine."Document Type", PurchOrderLine."Document No.", PurchOrderLine."Line No.", WorkDate(), SalesCommentLine.Comment);
    ...
    begin
        ...
        if ReqLineCheck.FindFirst()then repeat IsSpecial:=true;
                if ReqLineCheck.Type <> ReqLineCheck.Type::" " then IsOnlySpecialComments:=false;
            until ReqLineCheck.Next() = 0;
        if IsSpecial then if not IsOnlySpecialComments then begin
                UpdateReqLine.Reset;
                ...
                if UpdateReqLine.FindFirst()then begin
                    repeat UpdateReqLine.Validate("Accept Action Message", false);
    ...
    begin
        ...
        if PLine.FindLast()then NextLineNo:=PLine."Line No." + 10000
        else
            NextLineNo:=10000;
        SLine.Reset();
        ...
        if SLine.FindSet()then repeat ProcessSalesLine(SLine, PLine, NextLineNo, PurchaseHeader);
            until SLine.Next() = 0;
